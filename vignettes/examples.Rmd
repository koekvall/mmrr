...
```{r}
  # devtools::install_github("koekvall/lvmmr")
library(lvmmr)
```
...
Y <- generate_lvmmr(X = X, Beta = Beta_true, R = chol(Sigma_true),
...
fit <- lvmmr(Y = Y, X = X, type = type, M = M,
                           relative = T,
                           quiet = c(T, T, T, T),
...
D1 <- t(lvmmr:::get_cumulant_diffs(t(fit$W), type, 1))
D2 <- t(lvmmr:::get_cumulant_diffs(t(fit$W), type, 2))

lvmmr:::working_ll_rcpp(Y_T = t(Y), X_T = t(X), beta = fit$Beta,
                                  Sigma = fit$Sigma, W_T = t(fit$W),
                                  psi = psi_true,
                                  D1_T = t(D1), D2_T = t(D2))

lvmmr:::working_ll_rcpp(Y_T = t(Y), X_T = t(X), beta = Beta_MLE,
...
fit_uni <- lvmmr(Y = Y, X = X, type = type, M = M,
                           relative = FALSE,
                           quiet = c(F, T, T, T),
...
fit_trust <- lvmmr(Y = Y, X = X, type = type, M = M,
                           relative = FALSE,
                           quiet = c(F, T, T, T),
                           maxit = c(50, 100, 500, 100),
                           tol = c(1e-5, 1e-7, 1e-10, 1e-8),
                           ...
                           W = fit_uni$W)
# Use starting values
fit_pgd <- lvmmr(Y = Y, X = X, type = type, M = M,
                           relative = FALSE,
                           quiet = c(F, F, F, T),
                           maxit = c(50, 100, 500, 100),
                           tol = c(1e-5, 1e-7, 1e-10, 1e-8),
...
M <- diag(NA, r)
diag(M)[type == 2] <- 1
fit_diag <- lvmmr(Y = Y, X = X, type = type, M = M,
                           relative = FALSE,
                           quiet = c(F, T, T, T),
                           maxit = c(50, 100, 500, 100),
                           tol = c(1e-5, 1e-7, 1e-10, 1e-8),
...
Y_new <- generate_lvmmr(X = X_new, Beta = Beta_true, R = chol(Sigma_true),
                    type = type, psi = psi_true)
...
pred_GLM <- t(lvmmr:::get_cumulant_diffs(t(Xb_GLM), type, 1))

# We win.
pred_trust <- predict_lvmmr(X = X_new,
                                          Beta = fit_trust$Beta,
                                          sigma = sqrt(diag(fit_trust$Sigma)),
                                          type = type,
                                          num_nodes = 15)
pred_pgd <- predict_lvmmr(X = X_new,
                                          Beta = fit_pgd$Beta,
                                          sigma = sqrt(diag(fit_pgd$Sigma)),
                                          type = type,
                                          num_nodes = 15)
pred_diag <- predict_lvmmr(X = X_new,
                                          Beta = fit_diag$Beta,
                                          sigma = sqrt(diag(fit_diag$Sigma)),
                                          type = type,
                                          num_nodes = 15)
pred_oracle <- predict_lvmmr(X = X_new,
                                          Beta = Beta_true,
