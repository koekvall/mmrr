...
# Null hypothesis: Beta[1] = 0
X_null <- X[, 2, drop = F]

# Null is true
Beta_true <- rep(0, r)

# Variance parameters, psi treated as known, null is true
Sigma_true <- diag(0.5, r)
psi_true <- rep(0.5, r)
...
fit_full <- lvmmrPQL::lvmmr_PQL(Y = Y, X = X, type = type, M = M,
                           relative = T,
                           quiet = c(T, T, T, T),
                           maxit = c(100, 100, 500, 0),
                           tol = c(1e-12, 1e-8, 1e-10, 1e-8),
...
fit_null <- lvmmrPQL::lvmmr_PQL(Y = Y, X = X_null, type = type, M = M,
                           relative = T,
                           quiet = c(T, T, T, T),
                           maxit = c(100, 100, 500, 0),
                           tol = c(1e-12, 1e-8, 1e-10, 1e-8),
                           psi = psi_true,
                           pgd = FALSE,
                           Beta = fit_full$Beta[2],
                           Sigma = fit_full$Sigma)
p_val <- pchisq(q = lvmmrPQL:::lrt(fit_null = fit_null, fit_full = fit_full),
