...
# Each observation has one uniform predictor (SUR)
X <- as.matrix(Matrix::KhatriRao(matrix(runif(n * r, -1, 1), n, r),
                                 diag(1, r)))
Beta_true <- (r:1) / r

# Variance parameters, psi treated as known
Sigma_true <- 0.9^abs(outer(1:r, 1:r, FUN = "-"))
...
# No restrictions with normal responses
M <- matrix(NA, r, r)
diag(M)[type == 2] <- 1 # For identifiability (try)

fit <- lvmmrPQL::lvmmr_PQL(Y = Y, X = X, type = type, M = M,
                           relative = T,
                           quiet = c(F, F, F, T),
                           maxit = c(1000, 1000, 5000, 1000),
                           tol = c(1e-5, 1e-7, 1e-12, 1e-10),
                           psi = psi_true,
                           )
# Difference to truth
fit$Beta - Beta_true
fit$Sigma - Sigma_true
...
Y_new <- lvmmrPQL::generate_lvmmr(X = X_new, Beta = Beta_true, R = chol(Sigma_true),
                    type = type, psi = psi_true)
Beta_GLM <- c(
 coef(glm(Y[, 1] ~ 0 + X[seq(1, 2 * n,  by = 2), 1], family = "binomial")),
  coef(glm(Y[, 2] ~ 0 + X[seq(2, 2 * n,  by = 2), 2], family = "poisson")))
...
colMeans((Y_new - lvmmrPQL::predict_lvmmr(X = X_new,
                                          Beta = fit$Beta,
                                          sigma = sqrt(diag(fit$Sigma)),
                                          type = type,
                                          num_nodes = 15))^2)
colMeans((Y_new - pred_GLM)^2)
```
